  ...
  try {
    ...
    const reviewsResult = await pool.query(
      `SELECT
       ...
       GROUP BY tsr.id_review, u.username, u.profile_image_url, u.is_premium, u.subscription_plan
...
export const updateTvSeriesReview = async (req, res) => {
  try {
    ...
    return res.status(500).json({
      message: "Gagal mengubah review TV series",
      error: error.message,
    });
  }
};

export const deleteTvSeriesReview = async (req, res) => {
  try {
    const reviewId = parseReviewId(req.params.reviewId);
    ...
    await pool.query(`DELETE FROM flix.tv_series_reviews WHERE id_review = $1`, [
      reviewId,
    ]);
    ...
    return res.status(500).json({
      message: "Gagal menghapus review TV series",
      error: error.message,
    });
  }
};
